...
- Q: 用户认证与授权机制：系统应如何处理用户身份认证和资源访问授权？ → A: 简化Mock模式 - Mock数据无需认证，用户可直接访问所有mock资源，重点确保数据结构一致性
- Q: 企业列表排序逻辑：当多个企业都属于"相关资源企业"时，企业列表应如何排序显示？ → A: 按资源类型优先级排序 - 数据资源所属企业 > 模型所属企业 > 算力所属企业 > 其他企业（同类型内按名称字母序）
- Q: DAG数据持久化机制：用户在编排DAG过程中，数据应如何持久化保存？ → A: 内存存储+手动导出 - DAG仅存储在内存中，用户手动点击"导出"按钮生成JSON文件保存，刷新页面后数据丢失
...
- **页面刷新数据丢失**：DAG数据仅存储在内存中，用户刷新页面或关闭标签页后所有未导出的编排数据将丢失，系统应在用户尝试刷新时提示确认

## Requirements *(mandatory)*

### Functional Requirements

> **注意**: 本节列出系统必须满足的功能性需求。详细的用户场景和验收标准请参考对应的 User Story 章节。以下需求与 User Story 的对应关系：
> - 输入数据配置相关需求 → User Story 2
> - 计算模型配置相关需求 → User Story 3
> - 算力资源配置相关需求 → User Story 4
> - 输出数据配置相关需求 → User Story 5
> - 本地计算任务相关需求 → User Story 6
...
#### 输入数据配置
> 详细需求请参考 **User Story 2 - 配置输入数据与字段映射**
...
- **FR-029**: 系统必须在非表达式模型被添加时，弹出企业选择窗口，企业列表按优先级排序（参见 Clarifications L14 企业排序规则）
...
- **FR-035**: 系统必须在算力资源拖拽释放时弹出企业选择窗口，企业列表按优先级排序（参见 Clarifications L14 企业排序规则）
...
- **FR-040**: 系统必须在参与方企业选择中按优先级排序显示企业（参见 Clarifications L14 企业排序规则），也支持选择全网其他企业
...
#### 本地计算任务
- **FR-046**: 系统必须在用户拖拽"本地结果处理任务"到画布释放时弹出参与方企业选择窗口，企业列表按优先级排序（参见 Clarifications L14 企业排序规则）
...
#### Mock接口
- **FR-057**: 系统必须提供企业列表接口的mock数据（包含participantId、entityName），无需认证，用户可直接访问所有mock资源
- **FR-058**: 系统必须提供计算模型列表接口的mock数据（按type枚举值分类：CodeBin-V2、CodeBin-V3-1、CodeBin-V3-2、SPDZ），无需认证
- **FR-059**: 系统必须提供算力资源列表接口的mock数据（包含TEE板卡算力信息），无需认证
- **FR-060**: 系统必须提供数据资产字段接口的mock数据（包含 columnName、type、length、comments），无需认证
- **FR-061**: 系统必须确保mock数据结构与真实后端接口保持一致，便于后续切换到真实接口
...
- **SC-001**: 从拖拽第一个计算任务节点到画布开始计时，到完成配置输入数据、计算模型、算力资源并点击保存，总耗时不超过30秒
...
- **SC-007**: 用户无需查阅文档即可完成首次计算模型和算力资源配置（交互设计应直观自解释）
...
9. DAG数据仅存储在内存中，用户需要手动导出JSON文件来保存编排进度，刷新页面后数据将丢失
