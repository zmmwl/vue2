...
5. **Given** 用户正在选择流程中，**When** 用户点击取消或关闭对话框，**Then** 节点被删除或标记为未配置状态
...
- 当数据资产详情接口调用失败时，显示错误提示并提供重试选项
- 当网络请求超时时，显示超时提示并提供重试选项
...
- **FR-002**: 系统必须在数据资产选择对话框中支持三步骤选择流程：选择企业 → 选择数据资产 → 选择字段
...
- **FR-016**: 系统必须在未配置数据资产的节点上显示视觉标识（如"未配置"标签或特殊样式）
- **FR-017**: 系统必须在网络请求失败时显示友好的错误提示
- **FR-018**: 系统必须支持搜索和过滤企业列表及数据资产列表
- **FR-019**: 系统必须要求用户至少选择一个字段才能完成配置
- **FR-020**: 系统必须在用户取消配置时删除节点或将其标记为未配置状态
