...
import { BaseEdge, getSmoothStepPath, type EdgeProps, type Node } from '@vue-flow/core'

const props = defineProps<EdgeProps>()

// 从 Vue Flow 注入所有节点（用于智能路由）
...
const path = computed(() => {
  ...
  const pathResult = getSmoothStepPath({
    ...
    borderRadius: 8,
    offset: offset || undefined  // 仅在有偏移时传入
  })

  // getSmoothStepPath 返回字符串或元组，确保返回字符串
